...
| 구분 | 설명 |
|--|------|
|`문제상황`|setTimeout 함수를 이용한 문자 인증 만료 처리 시 <br>함수의 실행이 안되거나 실행이 지연되는 현상 발생  
|`문제원인`| *원인 1*  **예기치 못한 함수의 종료** <br> 인증번호 발급 후 배포가 이루어져야하는 상황<br>혹은 서버의 예기치 못한 종료가 발생하는 상황에서는<br>작업 대기 중이던 함수도 모두 종료됩니다.이렇게 되<br>면 setTimeout 함수의 이벤트가 실행되지 않은상태<br>에서 종료되어 데이터베이스 삭제 명령이 실행되지 않고<br> 인증번호가 그대로 데이터베이스 안에 남아있게 되어<br>인증번호 만료 시간인 3분이 지나더라도 인증이 가능한<br> 상황이 벌어집니다. <br>이는 인증시간을 3분 제한의 의미를 상실하게 만들뿐더러<br>인증 토큰을 탈취 후 공격을 가능함으로 보안 상으로도 <br>매우 좋지 않습니다.<br>*원인 2*  **호출 스택 내의 부하**     <br>동시간대에 가입하려는 사람들이 몰린 상황에서는<br>사람들이 인증 서비스를 사용하면 setTimeout<br>함수 스택이 계속해서 쌓일 것이고 물려있게 될 <br>것입니다. 이렇게 되면 처리 시간도 늘어나 서버<br>에 치명적입니다.
|`해결방안`|*방법 1*  **기존 방식 그대로 미결 건 남지 않게 신경 쓰기** <br>기존의 setTimeout 함수를 통한인증 만료 처리 방식 그대로 가져<br>가는 대신 엄격한 에러처리를 통해 미결 건 남지 않게 신경 쓰는 방법<br> *방법 2*   **DB 스케줄러를 통한 인증 만료 처리 구현하기**  <br>데이터베이스에 인증번호를 저장할 때 생성 시간도 함께 기록해 그것<br>을 바탕으로 만료시간 결정 하는 방법 |
|`결정`| DB 스케줄러를 통한 인증 만료 처리 구현|
|`결정이유`| 장점은 다음과 같습니다. <br>- 데이터의 서버 의존도 낮춤 <br>- 지속적인 이벤트 감시가 가능해 실시간 <br>이벤트 감시 및 로그 생성으로<br>중앙 서버에서 효율적 관리 도모

### [ 클라우드 스토리지 이미지 파일 업로드 및 로드 성능 개선 ]

| 구분 | 설명 |
|---|-------|
|`문제상황`|최초 가입시 클라우드 스토리지에 1장만 업로드 되는 대표 프로<br>필의 업로드 성능 약 100ms과 비교해 추가 프로필 업로드 시 <br>변경 가능 최대치인 5장을 설정하고 파일을 업로드했을 떄 평균<br>응답속도가 평균 응답 속도가 약 14s로 좋지 못했고 사용성과 <br>직결되는 부분이므로 반드시 개선이 필요했습니다.
|`해결방안`|- 이미지 업로드 부분 코드 리팩토링 및 이미지 리사이징 적용<br>|
|`결정&설계`| 전반적인 코드 리팩토링 과정 진행(이미지 업로드 로직의 간략화)<br>- 이미지 리사이징을 적용<br>- 읽기 및 쓰기 성능 최적화|