...
module.exports = async (req, res, next) => {
  ...
  if (!authToken || authType !== "Bearer") {res.status(401).send({ ok: 1, errorMessage: "비정상적인 활동이 감지되어 로그아웃됩니다(1)." });
    return;
  }
  ...
  try {
    ...
    if (authResult.message === "invalid signature") {// 위변조 검증
      res.status(401).send({ok: 2, errorMessage: "비정상적인 활동이 감지되어 로그아웃됩니다(2-1).",});
      return;
    }
    ...
    if (decoded === null) {      // 디코딩 결과가 없으면 권한이 없음을 응답. (=애시당초 잘못된 토큰)
      res.status(401).send({ ok: 2, errorMessage: "비정상적인 활동이 감지되어 로그아웃됩니다(2)." });
      return;
    }
    ...
    if (authResult.message === "jwt expired") {
      ...
      if (refreshResult.ok === false) { // 1. access token이 만료되고, refresh token도 만료 된 경우 => 새로 로그인해야. ()
        res.status(401).send({ ok: 3, errorMessage: "액세스만료, 리프레시만료. 로그아웃됩니다(3).", });
        ...
        if (token.accessToken !== authToken) {
          ...
          res.status(401).send({ ok: 4, errorMessage: "만료된 토큰으로 한번만 재발급이 가능합니다. 로그아웃됩니다(4).", });
    ...
    res.status(401).send({ ok: 5, errorMessage: "로그인 후 이용 가능한 기능입니다(5)." });
  }
};

function verify(token) { // token 검증 (1.위변조 + 2.만료를 한번에 검증 / 위변조가 되어도 에러, 만료가 되어도 에러, 메세지는 다름.)
  let decoded = null;
  try {
