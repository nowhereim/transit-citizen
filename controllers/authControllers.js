...
class AuthControllers {
    ...
    getUserPhoneNumber = async (req, res) => {
        try {
            ...
            await this
                    .authServices
                    .sendAuthorityCheckMessage( phoneNumber );
            return res
                    .status(200)
                    .send({ msg: '인증번호가 전송 되었습니다' });
                } catch (error) {
        ...
        try {
            ...
            if ( !phoneNumber || !authCode)
                return res
                        .status(400)
                        .send({ error: "잘못된 형식입니다"});
            if (( phoneNumber.search(phoneNumberCheck) || authCode.search(authCheck) ) === -1) {
                return res
                        .status(400)
                        .send({ error: "잘못된 인증번호 입니다"});
            }
            const isEmpty 
                = await this
                        .authServices
                        .checkAuthNumber( phoneNumber, authCode );
            if ( isEmpty === null )
                return res
                        .status(400)
                        .send({ error: "인증에 실패하였습니다" });
            return res
                    .status(200)
                    .send({ msg: "인증되었습니다" });
        } catch (error) {
